    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const allSubmissions = axiosResponseSubmissionHistories.data.data;
                ...
                if (
                    localUser?.id &&
                    localUser?.roles?.some((r: any) => r.name === "agent")
                ) {
                    const filteredSubmissions = allSubmissions.filter(
                        (item: any) => item.user_id === localUser.id
                    );
                    setSubmissionHistoriesData(filteredSubmissions);
                } else {
                    setSubmissionHistoriesData([]);
                }
            } catch (error) {
